...
function renderEquipment(){
  ...
  $('equipmentBar').innerHTML=items.map(([type,label,name,detail,images])=>{
    const level=state[type],visual=images?`<img src="${images[level-1]}" alt="">`:['hooks','knives'].includes(type)?gearIcon(type,level):`<span class="gear-emoji" aria-hidden="true">${type==='baits'?baitIcons[bait.id]:'📡'}</span>`;
...
function dailyMarket(){const day=new Date().toISOString().slice(0,10);if(state.market?.day===day)return state.market;const seed=[...day].reduce((sum,char)=>sum+char.charCodeAt(0),0),names=Object.keys(fishPrices);state.market={day,fish:names[seed%names.length],bonus:.3+(seed%51)/100};return state.market}
...
const eventBite=()=>{const event=eventData[world.event];return event.nightOnly&&world.time!=='night'?.65:event.bite};
...
function ensureNpcQuest(){const today=new Date().toISOString().slice(0,10);if(!state.npcQuest||state.npcQuest.day!==today||!fishWeights[state.npcQuest.fish]||typeof state.npcQuest.reward!=='object'||!activePool().some(fish=>fish.name===state.npcQuest.fish))state.npcQuest=makeNpcQuest();return state.npcQuest}
...
class ProceduralScene {
  ...
  startCast(power){this.cast=power;this.reel=0;this.x=31+power*.55;this.y=60;this.biting=false;this.fish=null;$('equippedFloat').hidden=true;this.fightOffset={x:0,y:0,targetX:0,targetY:0,next:0}}
  clear(){this.cast=0;this.reel=0;this.tension=0;this.biting=false;this.pulling=false;this.fish=null;$('equippedFloat').hidden=true;this.fightOffset={x:0,y:0,targetX:0,targetY:0,next:0}}
  floatPoint(w,h,now=performance.now()){const p=this.pulling?this.reel/100:0,base={x:w*(this.x+(56-this.x)*p)/100,y:h*(this.y+(84-this.y)*p)/100};if(this.pulling&&this.fish){const resistance=Math.min(1,Math.max(.2,this.fish.weight/Math.max(1,tackleLimits().rod))),amplitude=(7+resistance*24)*(1-p);if(now>=this.fightOffset.next){this.fightOffset.targetX=(Math.random()*2-1)*amplitude;this.fightOffset.targetY=(Math.random()*2-1)*amplitude*.65;this.fightOffset.next=now+Math.max(65,190-resistance*105)}this.fightOffset.x+=(this.fightOffset.targetX-this.fightOffset.x)*.13;this.fightOffset.y+=(this.fightOffset.targetY-this.fightOffset.y)*.13}else{this.fightOffset.x*=.82;this.fightOffset.y*=.82}return{x:Math.max(w*.05,Math.min(w*.95,base.x+this.fightOffset.x)),y:Math.max(h*.53,Math.min(h*.91,base.y+this.fightOffset.y))}}
  // Follow the artwork in its actual layout, including narrow screens.
  rodTip(w,h){const image=$('equippedRodImage'),rect=image.getBoundingClientRect(),canvas=this.c.getBoundingClientRect(),scale=Math.min(rect.width/(image.naturalWidth||1536),rect.height/(image.naturalHeight||1024)),width=(image.naturalWidth||1536)*scale,height=(image.naturalHeight||1024)*scale;return {x:rect.left-canvas.left+(rect.width-width)/2+width*.97,y:rect.top-canvas.top+(rect.height-height)/2+height*.05}}
  frame(now){const t=(now-this.t0)/1000,g=this.g,w=this.w,h=this.h;g.clearRect(0,0,w,h);this.ripples(g,w,h,t,now);this.line(g,w,h,now);if(this.cast)this.bobber(g,w,h,t,now);requestAnimationFrame(n=>this.frame(n))}
  ...
  line(g,w,h,now){if(!this.cast)return;const p=this.floatPoint(w,h,now),tip=this.rodTip(w,h);g.save();g.strokeStyle=this.tension>82?'#ff8a7d':this.tension>60?'#ffe078':lineColors[state.lines-1];g.lineWidth=.9+state.lines*.16+this.tension/100;g.beginPath();g.moveTo(tip.x,tip.y);g.lineTo(p.x,p.y);g.stroke();g.restore()}
  ...
  bobber(g,w,h,t,now){const p=this.floatPoint(w,h,now),float=$('equippedFloat');float.hidden=false;float.style.left=p.x+'px';float.style.top=(p.y+Math.sin(t*(this.biting?15:4))*(this.biting?9:3))+'px';float.classList.toggle('night-float',state.floats>=4&&world.time==='night')}
...
function pickFish(){
  ...
  const boss=availableBoss();if(boss&&Math.random()<.025){const weight=+(boss.weight[0]+Math.random()*(boss.weight[1]-boss.weight[0])).toFixed(2);return {id:Date.now()+Math.random(),name:boss.name,rarity:5,weight,pricePerKg:boss.pricePerKg,clean:false,cuts:0,location:state.location,boss:boss.id,image:fishImages[boss.base]}}
...
function bite(){if(!casting)return;sceneRenderer.biting=true;if(Math.random()<Math.min(.42,Math.max(.05,.18/worldActivity()))){note('Ложная поклёвка — рыба ушла с приманкой.');biteTimer=setTimeout(()=>loseFish('Ложная поклёвка: улова нет.'),900);return}sceneRenderer.fish=pickFish();updateFishingControls();note('Клює! Утримуй «Підтягнути», ЛКМ або R.');biteTimer=setTimeout(()=>loseFish('Рыба сорвалась: вы не успели подтянуть снасть.'),6200)}
function startPull(e){if(e.button===0&&!e.target.closest('button,input,select,.modal,.butcher-overlay'))beginPull()}
function beginPull(){if(!casting||pulling||fishingBlocked())return;const fish=sceneRenderer.fish;if(!fish)return;const limits=tackleLimits();if(fish.weight>limits.hook){loseFish('Крючок слишком мал для этого улова.');return}if(fish.weight>limits.rod){loseFish('Удилище не выдержало вес улова.');return}pulling=true;sceneRenderer.pulling=true;$('equippedRod').classList.add('pulling');updateFishingControls();note(`Подтягивайте снасть: ${fishWeight(fish)} на крючке.`);pullTimer=setInterval(()=>{reel=Math.min(100,reel+8*limits.reel);sceneRenderer.reel=reel;updateDepth(true);$('depthNeedle').style.left=(20+reel*.7)+'%';const load=fightLoad(fish,reel);if(setTension(load)>=1){loseFish(load>limits.line?'Леска оборвалась от натяга.':'Удилище не выдержало вес рыбы.');return}if(reel>=100)landFish()},80)}
function stopPull(e){if(e.button!==0||!pulling)return;pulling=false;sceneRenderer.pulling=false;$('equippedRod').classList.remove('pulling');clearInterval(pullTimer);updateFishingControls();const fish=sceneRenderer.fish;if(fish)setTension(fightLoad(fish,reel)*.65);if(casting)note('Риба пручається — продовжуй підтягувати.')}
function loseFish(reason){if(!casting)return;perfectCast=false;casting=false;pulling=false;clearInterval(pullTimer);clearTimeout(biteTimer);$('equippedRod').classList.remove('pulling');sceneRenderer.clear();setTension();resetTournamentStreak();state.journal.unshift(reason);state.journal=state.journal.slice(0,30);note(reason);render()}
function landFish(){
